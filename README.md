...
インフラエンジニア → SaaSファウンダー。AWSコスト最適化とセキュリティの自動化ツールを開発中。
...
AWSコストをClaude AIが分析し、日本語で削減提案を自動生成。RI稟議書もワンクリックでPDF出力。
...
S3・IAMの設定ミスを自動検出。8種類のルール、ReadOnly接続のみで安全。
...
`Python` `boto3` `Claude API` `pytest` `rich` `GitHub Actions`
